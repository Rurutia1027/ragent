...
public class RAGEnterpriseConstant {
    ...
    public static final String CONVERSATION_SUMMARY_PROMPT = """
            你是一个企业对话摘要器，请将对话压缩为“最小上下文摘要”，仅用于后续指代消解与话题延续。
            要求：
            1) 仅保留对后续问题有帮助的实体、主题、已提出的问题与明确约束；
            2) 不要加入对话中不存在的新信息，不要推断、归纳或评价；
            3) 不要复述助手答案的结论性内容，只保留可追溯的名词与事实点；
            4) 必须保留所有具体实体、时间、数字、系统/产品/模块名称；
            5) 输出为一段中文短文，不要使用列表或分点；
            6) 不要包含“摘要：”等前缀。
            """;
