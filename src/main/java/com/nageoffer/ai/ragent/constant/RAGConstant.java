...
public class RAGConstant {
    ...
    public static final String QUERY_REWRITE_AND_SPLIT_PROMPT = """
            ...
            你的任务（只做这两件事，不要回答问题）：
            1) 将用户问题改写成适合检索的【单条简洁查询】（保留专有名词和关键约束）。
    ...
    /**
     * 意图识别提示词模板（并行模式 - 单 Domain）
     * 每次只发送某个 Domain 下的叶子意图节点，做精细判断
     * 占位符顺序：Domain名称、Domain描述、分类列表、用户问题
     */
    public static final String INTENT_CLASSIFIER_DOMAIN_PROMPT = """
            你是企业/组织内部知识库的【意图分类助手】。你的输出用于路由知识库/工具/系统对话，**误报的代价远高于漏报**：宁可返回 `[]` 也不要勉强匹配。
            
            ### 【当前领域】
            你现在只需要判断用户问题是否与「%s」这个领域相关。
            领域说明：%s
            
            ### 【输入说明】
            1. 下面给出该领域下若干“最终分类节点”（叶子节点），它们挂接了各自的知识库/文档/工具/系统对话能力。
            2. 用户问题可能包含多个子问题（多句、多问、多个“？”等）。
            3. 每个节点可能包含（以实际提供为准）：
               - `id`：唯一标识
               - `fullPath/path`：完整路径
               - `name`：名称
               - `description`：覆盖范围说明
               - `examples`：典型问法
               - `kind/type`：分类类型（如 KB / MCP / SYSTEM）
            
            ### 【任务】
            - 判断用户问题（及其子问题）是否与本领域某些节点相关。
            - **明显无关**：直接返回 `[]`。
            - **确实相关**：返回匹配节点（可多个，但必须“最少且必要”）。
            
            ---
            
            ### 【硬性流程（必须逐步执行）】
            #### A. 子问题切分
            将用户问题按 `？`、`。`、`；`、`;`、换行切成多个子问题，逐条判断。
            
            #### B. 领域相关性闸门（先过闸门再匹配节点）
            - 若所有子问题都与本领域明显无关：**必须输出 `[]`**。
            - 不要因为“词很泛（如：功能、怎么用、地址、配置）”就认为与领域相关。
            
            #### C. 节点匹配（证据匹配 + 最少集合）
            对每个子问题：
            1) 先判断它是否属于本领域；
            2) 若属于本领域，再从节点中选 **1 个最合适** 的节点；
            3) 最终输出的节点集合要满足：**覆盖所有相关子问题的同时，节点数量尽量少**（能用更上位/更通用节点覆盖时，不要额外返回更窄的节点）。
            ...
            #### 1) “证据匹配”是唯一允许命中的理由
            只有当你能清楚复述以下证据时，才允许匹配某节点：
            - 子问题里出现的 **关键字/实体** ↔ 节点的 `fullPath/path/name/description/examples` 中出现的 **对应词**（同词或高度同义词）
            
            **禁止**仅凭“同领域”“看起来相关”“业务上可能有关”来匹配。
            
            ---
            
            #### 2) 实体导向优先（强约束）
            只要子问题中出现“具体命名对象”，必须按实体导向处理：
            - “具体命名对象”包括但不限于：系统/产品/应用/平台/服务/模块/接口/项目/环境/资源/账号/权限/配置项的名称或标识
            - 以及常见标识：URL/域名/IP/端口/路径/实例ID/集群名/租户名/工单号/错误码/日志关键字等
            
            实体导向的硬要求：
            - **对象名（或明确同义词/别名）必须出现在节点字段中**（`id/fullPath/path/name/description/examples` 任一处），否则不得匹配该节点。
            
            ---
            
            #### 3) 主题导向（仅在无明确对象名时允许）
            当子问题不包含具体命名对象时，才允许主题导向匹配：
            - 必须命中节点描述/路径/示例中的 **主题特征词**（与该节点强相关的关键词）
            - 仅出现泛词不算主题特征词：例如“功能/介绍/说明/怎么用/流程/配置/地址/参数/报错/打不开”等泛词 **不能作为命中证据**
            
            > 重要：
            > **更窄的节点（例如安全/权限/审计/合规/计费/性能等）只能在子问题明确提到对应主题特征词时才可匹配。**
            > 仅仅问“主要功能/有哪些能力/怎么用”这类泛问，应该优先匹配更通用的“系统介绍/使用指南/常见问题”等节点，而不是匹配到更窄主题节点。
            
            ---
            
            #### 4) SYSTEM 类节点（type/kind=SYSTEM）严格门槛（防误触发）
            仅当子问题明确在问“对话助手本身”时，才允许匹配 SYSTEM：
            - 允许命中的典型方向：助手是谁、能做什么、怎么使用助手、能力边界、权限/隐私/记忆规则、支持哪些操作等
            - **必须出现明确指向助手的称呼/指代**（例如“你/助手/机器人/AI/模型/本助手”等）或明确元问题意图（如“你是谁/你能做什么/怎么用你/你支持什么/你的限制/你如何处理数据”等）
            
            禁止匹配 SYSTEM 的情况：
            - 子问题在询问任何“非助手对象”（任意系统/服务/模块/接口/环境/资源/账号权限/配置/故障排查/地址参数等）
            - 若同一子问题同时出现“助手指代词”与“非助手对象名”，以非助手对象为主：**不得匹配 SYSTEM**
            
            ---
            
            #### 5) 去重与合并（解决重复 id）
            - **输出中每个 `id` 只能出现一次**（严禁重复）。
            - 若多个子问题都指向同一 `id`：只保留一个对象
              - `score` 取这些匹配中的 **最大值**
              - `reason` 合并为一句（用 `；` 串联，指出分别覆盖了哪些子问题证据）
            
            ---
            
            #### 6) 低分禁止输出（硬阈值）
            - **不允许输出 `score < 0.4` 的节点**
            - 如果所有可能匹配都达不到 0.4：**必须返回 `[]`**
            
            ---
            
            ### 【打分原则】
            - `> 0.8`：证据非常明确（实体/主题特征词清晰命中，范围高度一致）
            - `0.4 ~ 0.8`：有证据但不完整（仍必须能明确说出命中证据）
            - `< 0.4`：禁止输出
            ...
            2. 数组元素格式（仅这三个字段）：
               - `id`：必须来自分类列表中的某个 id
               - `score`：0~1 小数
               - `reason`：中文简述；必须包含“命中的子问题片段 + 命中证据（问题词 ↔ 节点字段词）”
            3. 若无匹配：输出 `[]`
            4. 输出按 `score` 从高到低排序。
