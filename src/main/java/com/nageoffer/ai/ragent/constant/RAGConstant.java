...
public class RAGConstant {
    ...
    public static final String INTENT_CLASSIFIER_DOMAIN_PROMPT = """
            ...
            【任务说明】
            1. 下面会给出该领域下的若干"最终分类节点"（叶子节点），它们都挂接了各自的知识库/文档集合。
            2. 你的任务是：判断用户问题是否与这个领域的某个分类相关。
            3. 如果用户问题明显与该领域无关，直接返回空数组 `[]`。
            4. 只有当问题确实与该领域相关时，才返回匹配的分类。
            
            【判断标准】
            1. 先判断问题是否属于该领域：
               - 如果问题中提到的系统/产品/模块/主题与该领域完全无关，直接返回 `[]`
               - 不要勉强匹配，宁可返回空数组
            
            2. 对于【实体导向问题】（问题中出现具体系统/产品/模块名称）：
               - 只有当问题中的关键实体在分类的 id/path/description/examples 中出现时，才认为相关
               - 仅仅是「业务领域类似」不算匹配
            
            3. 对于【主题导向问题】（没有具体系统名称，围绕某个主题）：
               - 如果分类的 path/description/examples 中直接包含相同或高度同义的主题词，可以视为相关
            
            4. 每个节点包含：
               - id：唯一标识
               - path：在分类树中的完整路径
               - description：该分类覆盖的知识范围
               - type：分类类型（KB=知识库 / MCP=工具调用 / SYSTEM=系统对话）
               - examples：该分类下典型的用户提问
            ...
            2. 数组中的每个元素是一个对象，字段为：
               ...
               - score：0 到 1 之间的小数
               - reason：简要中文说明为什么匹配
            3. 如果该领域下的所有分类都与问题无关，返回空数组 `[]`。
            4. score 打分原则：
               - > 0.8：关键实体明确匹配，场景高度吻合
               - 0.4~0.8：有一定相关，但关键实体不完全一致
               - < 0.4：勉强沿边，建议返回空数组
            
            示例输出：
            [{"id": "biz-oa-intro", "score": 0.92, "reason": "问题询问 OA 系统整体功能，贴合该分类"}]
